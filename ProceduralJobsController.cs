...
namespace DVOwnership
{
    public class ProceduralJobsController
    {
        ...
        public IEnumerator GenerateJobsCoro(Action onComplete)
        {
            ...
            var licensedOutputCargoGroups = (from cargoGroup in proceduralRuleset.outputCargoGroups where LicenseManager_Patches.IsLicensedForCargoType(cargoGroup.cargoTypes.ToHashSet<CargoType>()) select cargoGroup).ToList();
            ...
            lock (RollingStockManager.syncLock)
            {
                ...
                foreach (var track in stationTracks)
                {
                    ...
                    {
                        if (Utilities.IsAnySpecialCar(types.TrainCarType_to_v2.FirstOrDefault(x => x.Value == car.carType).Key)) { continue; }
                        carsInYard.Add(car);
                    }
                ...
                var activeJobs = jobManager.currentJobs;
