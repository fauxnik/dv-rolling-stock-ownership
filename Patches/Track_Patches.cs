...
namespace DVOwnership.Patches
{
	public class Track_Patches
	{
		...
		static void OccupiedLength_get_Postfix(Track __instance, ref float __result)
		{
			...
			var lengthOfEquipment = (from equipment in equipmentOnTrack select yto.GetCarTypeLength(equipment.CarType)).Sum();
			var occupiedLength = lengthOfEquipment + yto.GetSeparationLengthBetweenCars(equipmentOnTrack.Count());
