...
public class PlayerManager_Patches
{
	...
	public static void Setup()
	{
		if (isSetup)
		{
			Main.LogWarning("Trying to set up player manager patches, but they've already been set up!");
			return;
		}

		Main.Log("Setting up PlayerManager patches.");
		...
		Main.Patch(PlayerManager_SetCar, prefix: new HarmonyMethod(PlayerManager_SetCar_Prefix), postfix: new HarmonyMethod(PlayerManager_SetCar_Postfix));
	}
	...
	static void SetCar_Postfix(TrainCar __state, ref TrainCar ___LastLoco)
	{
		...
		{
			Main.Log($"Restoring {prevLoco} as Player's last loco, because it is moving and {nextLoco} is not.");
